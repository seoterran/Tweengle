...
!(function(exports, undefined) {
  ...
  exports.InvitationList = Backbone.Collection.extend({
    ...
    initialize: function(models, options) {
      ...
      this.dispatcher.on('inviteRemoteUserFree', this.inviteRemoteUserFree, this);
      this.dispatcher.on('remoteUserBecameUnavailable', this.remoteUserBecameUnavailable, this);
    },

    inviteRemoteUser2: function(remoteUser,freeCall){
          var  self=this,
               outgoingInvitation;
        
          console.log('InvitationList inviteRemoteUser2: '+freeCall);
        
          outgoingInvitation = new Invitation({ //will be displayed 
            incoming: false,
            remoteUser: remoteUser,
            free: freeCall       
           // rate: currentRate //to display updated rate if needed
          });
          ...
          outgoingInvitation.save({}, {
            success: function() {
              // Now the invitation is ready to be sent to the remote user via OpenTok signalling
                console.log('invitation ready to be sent : '+remoteUser.get('rate'));
     ...
     inviteRemoteUserFree: function(remoteUser) {
          console.log('InvitationList inviteRemoteUserFree: ');
          this.inviteRemoteUser2(remoteUser,true);
     },
    ...
    inviteRemoteUser: function(remoteUser) {
      ...
      var myID=this.dispatcher.me.get('uid');
      var remoteUserID= remoteUser.get('uid');   
      $.get('/timeCheck', { uid1: myID, uid2: remoteUserID})
            .done(function(data) {
              //self.offlineCollection.set(data);
              console.log('InvitationList time check: done '+myID+'/'+remoteUserID+'/'+data+'/'+data.timeRemaining+'/'+data.rate);
              var temp=0;
              if(data.timeRemaining!=null){
                  console.log('data.timeRemaining!=null');
                  temp=data.timeRemaining;
              }else
                   console.log('data.timeRemaining==null');
          
              if(temp<5){
                  console.log('not enough time remaining ');
                  alert('현재 사용하실 수 있는 시간은 '+ temp+'분 입니다. 통화를 시작하기 위해서는 최소 5분 이상이 필요합니다');
                  return;
              } else{
                 // currentRate=data.rate;
                   remoteUser.set('timeRemaining',data.timeRemaining); //update rate if needed
                 //  console.log('you can chat for '+data.myPoint*60/data.rate+' minutes'+'/'+remoteUser.get('rate'));
                   
                   self.inviteRemoteUser2(remoteUser,false);
              }
          
             /* var temp=data.myPoint - (data.rate*5/60);
              
              if(temp<0){
                  console.log('not enough point ');
                  alert('현재 보유하신 포인트로는 '+data.myPoint*60/data.rate+'분 밖에 할 수 없습니다. 과외를 시작하기 위해서는 최소 5분이상 할 수 있는 포인트가 필요합니다');
                  return;
              } else{
                 // currentRate=data.rate;
                   remoteUser.set('rate',data.rate); //update rate if needed
                   console.log('you can chat for '+data.myPoint*60/data.rate+' minutes'+'/'+remoteUser.get('rate'));
                   
                   self.inviteRemoteUser2(remoteUser);
              }
              */
            })
            .fail(function(jqXHR, textStatus, errorThrown) {
              console.log('invitationList timeCheck: failed', errorThrown);
              return;
             // fail(errorThrown);
            });
      ...
      this.dispatcher.once('userAvailability', function(available) {
        ...
        if (available) {
          ...
          self.dispatcher.once('remoteUser~'+event.from.connectionId, function(remoteUser) {
            ...
            console.log('InvitationList receiveInvitation freeCall: '+incomingInvitation.get('free'));
